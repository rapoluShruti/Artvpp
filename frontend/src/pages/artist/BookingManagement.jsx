...
const BookingManagement = () => {
  ...
  const fetchBookings = async () => {
    try {
      const res = await axios.get('http://localhost:5000/api/creative-services/bookings/artist/all', {
  ...
  const handleApproveBooking = async (bookingId) => {
    try {
      await axios.put(`http://localhost:5000/api/creative-services/bookings/${bookingId}/approve`, 
        {},
        { headers: { Authorization: `Bearer ${token}` } }
      );
      fetchBookings();
  ...
  return (
    <div className="min-h-screen bg-white">
      <div className="max-w-6xl mx-auto px-4 py-12">
        {/* Header */}
        <div className="mb-8">
          <h1 className="text-4xl font-bold text-black mb-2">Booking Management</h1>
          <p className="text-gray-600">Manage and approve event bookings</p>
        </div>
        ...
        {filteredBookings.length === 0 ? (
          <div className="text-center py-16 border-2 border-gray-300 rounded-lg bg-gray-50">
            ...
            <p className="text-xl font-bold text-gray-600">No bookings</p>
          </div>
        ) : (
              ...
              <div key={booking.id} className={`border-2 rounded-lg p-6 bg-white transition-all ${getStatusColor(booking.status)}`}>
                ...
                <div className="flex flex-col md:flex-row md:items-center md:justify-between gap-4 mb-4 pb-4 border-b-2 border-gray-200">
                  <div>
                    ...
                    <div className="text-sm text-gray-600 mt-1">From: <span className="font-bold">{booking.first_name} {booking.last_name}</span></div>
                    <div className="text-sm text-gray-600">Email: <span className="font-bold">{booking.email}</span></div>
                  </div>
                ...
                <div className="grid grid-cols-2 md:grid-cols-4 gap-4 mb-4 pb-4 border-b-2 border-gray-200">
                  ...
                  <div className="bg-white p-3 rounded-lg border-2 border-gray-200">
                    <div className="text-xs font-bold text-gray-600 mb-1">Requested</div>
                    <div className="font-bold text-black">{new Date(booking.created_at).toLocaleDateString()}</div>
                  </div>
                </div>
